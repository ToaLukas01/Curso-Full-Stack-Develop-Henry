...
function newtonRaphson(x, count){
  ...
  var varloAnterior = x/2;
  var resultado = 0;
  while(count !== 0){
    resultado = (varloAnterior + (x / varloAnterior))/ 2;
    count -= 1;
    return resultado + (newtonRaphson(x, count));
  }
  return resultado



  //if(x < 0) return -1 // Si es negativo
  //if(x % 1 !== 0)  return -1 // Si no es entero
  //var valorAnterior = x/2
  //var result = 0;

  //while(count >0) {
    //result = (valorAnterior + (x/valorAnterior)) / 2
...
//if(!current.next){
  //nuevaLista.add(this.head.value)
...
LinkedList.prototype.insertInOrder = function(value){
    ...
    //if(this.head === null || nuevo.value > this.head.value){
      //nuevo.next = this.head
      //this.head = nuevo
    //} else{
      //let current = this.head
      //while(current.next && nuevo.value < current.next.value)
        //current = current.next
  ...
  //return function(user){
    ...
    //for(var i = 0 ; i< flights.length ; i++){
      //if(flights[i].origin === user.origin){
        //for(var j =0 ; j< flights[i].destinations.length; j++){
          //if(flights[i].destinations[j].miles <= user.miles){
            //ciudades.push(flights[i].destinations[j].city)
          //}
        //}
      //}
    //}
