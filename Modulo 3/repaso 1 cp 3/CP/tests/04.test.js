...
describe("---------- `play` ----------", function () {
  ...
  it("Si el usuario no existe, arroja un error", function () {
    expect(() => Model.play("The Office", "elena@henryflix.com")).to.throw(
      "Usuario inexistente"
    );
  });
  ...
  it('Si la serie es "premium" y el usuario no tiene dicho plan, arroja un error', function () {
    ...
    expect(() =>
      Model.play("How I Met Your Mother", "elena@henryflix.com")
    ).to.throw("Contenido no disponible, contrata ahora HenryFlix Premium!");
  });
  ...
  it('"Reproduce" una serie y devuelve un mensaje de confirmación', function () {
    ...
    Model.addSerie("How I Met Your Mother", 9, "premium", 2005);
    Model.addUser("elena@henryflix.com", "Elena");
    expect(Model.play("The Office", "elena@henryflix.com")).to.eql(
      "Reproduciendo The Office"
    );
    Model.switchPlan("elena@henryflix.com");
    expect(Model.play("How I Met Your Mother", "elena@henryflix.com")).to.eql(
      "Reproduciendo How I Met Your Mother"
    );
  });
