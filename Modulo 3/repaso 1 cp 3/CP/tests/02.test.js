...
describe("---------- `switchPlan` ----------", function () {
  ...
  it("Cambia el plan del usuario indicado (regular >> premium >> regular)", function () {
    ...
    expect(Model.switchPlan("wan@henryflix.com")).to.eql(
      "Wanda, ahora tienes el plan premium"
    );
    expect(Model.switchPlan("wan@henryflix.com")).to.eql(
      "Wanda, ahora tienes el plan regular"
    );
  });

  it("Si el usuario no existe, arroja un error", function () {
    expect(() => Model.switchPlan("wan@henryflix.com")).to.throw(
      "Usuario inexistente"
    );
  });
});
