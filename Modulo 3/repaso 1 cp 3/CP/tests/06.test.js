...
describe("---------- `rateSerie` ----------", function () {
  ...
  it("Actualiza el puntaje de la serie y devuelve un mensaje de confirmación", function () {
    ...
    expect(Model.rateSerie("The Office", "martu@henryflix.com", 5)).to.eql(
      "Le has dado 5 puntos a la serie The Office"
    );
    expect(Model.rateSerie("The Office", "mora@henryflix.com", 3)).to.eql(
      "Le has dado 3 puntos a la serie The Office"
    );
    expect(Model.rateSerie("The Office", "wan@henryflix.com", 4)).to.eql(
      "Le has dado 4 puntos a la serie The Office"
    );
  ...
  it("Si el usuario no existe, arroja un error y no actualiza el puntaje", function () {
    ...
    Model.addSerie("The Office", 9, "regular", 2005);
    expect(() =>
      Model.rateSerie("The Office", "flor@henryflix.com", 5)
    ).to.throw("Usuario inexistente");
  ...
  it("Si la serie no existe, arroja un error y no actualiza el puntaje", function () {
    ...
    expect(() =>
      Model.rateSerie("The Office", "martu@henryflix.com", 5)
    ).to.throw("Serie inexistente");
    expect(Model.listSeries()[0].reviews).to.eql([]);
    expect(Model.listSeries()[0].rating).to.eql(0);
  });
  ...
  it("Si el puntaje no es de 1 a 5, arroja un error y no actualiza el puntaje", function () {
    ...
    Model.addSerie("The Office", 9, "regular", 2005);
    expect(() =>
      Model.rateSerie("The Office", "martu@henryflix.com", 6)
    ).to.throw("Puntaje inválido");
    expect(Model.listSeries()[0].reviews).to.eql([]);
    expect(Model.listSeries()[0].rating).to.eql(0);
  });
  ...
  it("Si el usuario no reprodujo la serie, devuelve un error y no actualiza el puntaje", function () {
    ...
    expect(() =>
      Model.rateSerie("The Office", "martu@henryflix.com", 5)
    ).to.throw("Debes reproducir el contenido para poder puntuarlo");
    expect(Model.listSeries()[0].reviews).to.eql([]);
    expect(Model.listSeries()[0].rating).to.eql(0);
  });
});
