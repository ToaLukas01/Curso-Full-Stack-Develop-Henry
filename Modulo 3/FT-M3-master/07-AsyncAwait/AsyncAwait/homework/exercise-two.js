...
async function problemA () {
  ...
  // AsyncAwait version
  async function poema2A (){
    const poema1 = await promisifiedReadFile('poem-two/stanza-01.txt')
    const poema2 = await promisifiedReadFile('poem-two/stanza-02.txt')
    blue(poema1)
    blue(poema2)
  }
  await poema2A()
...
async function problemB () {
   ...
   *    (ignora errores)
   *    nota: las lecturas ocurren en paralelo (en simultaneo) //cuando piden en paralelo = Promise.all()
   *
   */
  ...
  async function lecturas(fil){
    const poema = await promisifiedReadFile(fil)
    blue(poema)
  }
  await Promise.all(filenames.map((poema)=> lecturas(poema)))
  console.log("done")
  // callback version
