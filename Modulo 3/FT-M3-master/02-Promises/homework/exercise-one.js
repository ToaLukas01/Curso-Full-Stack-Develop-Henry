...
function problemB () {
  ...
  // promise version
  let poemas = ['poem-one/stanza-02.txt', 'poem-one/stanza-03.txt'];
  let arr = poemas.map(p => promisifiedReadFile(p).then(s =>{
...
function problemC () {
  ...
  .then(s2 => {
    console.log('-- C. PROMESA version (stanza two) --');
    blue(s2);
    return promisifiedReadFile('poem-one/stanza-03.txt')
  }).then(s3 => {
    console.log('-- C. PROMESA version (stanza three) --');
    blue(s3);
    console.log('-- C. done --');
  })
}
...
function problemE () {
  ...
  .then(s3 => {
    console.log('-- E. PROMESA version (stanza three) --');
    blue(s3);
    return promisifiedReadFile('poem-one/stanza-04.txt')
  }).then(s4 => {
    console.log('-- E. PROMESA version (stanza four) --');
    blue(s4);
  })
